...
- **SPADE: Self-Play in Adaptive Synthetic Executable Environments** — 2026. [arXiv:2608.19197](https://arxiv.org/abs/2608.19197). One model both writes executable training environments and learns to act in them, with the designer rewarded for environments at the edge of the agent's ability; +5.3 on average over the strongest fixed-environment baseline across eight held-out benchmarks at 30B.
- **G-Zero: Self-Play for Open-Ended Generation from Zero Data** — 2026. [arXiv:2605.09959](https://arxiv.org/abs/2605.09959). Verifier-free co-evolution for open-ended tasks: a proposer targets the generator's blind spots using the shift a self-generated hint causes in the generator's answer, so no external judge caps what can be learned.
- **From RLVR to RLSVR: Task Transformation Induces Self-Verifiable Rewards for Open-Ended LLM Self-Improvement** — COLM 2026. [arXiv:2607.23802](https://arxiv.org/abs/2607.23802). Turns open-ended tasks into verifiable games — in SpyRL, agents vote out a planted spy whose identity is known — so RLVR-style self-improvement reaches summarization and creative writing without an LLM judge.
...
- **Huxley-Gödel Machine: Human-Level Coding Agent Development by an Approximation of the Optimal Self-Improving Machine** — 2025. [arXiv:2510.21614](https://arxiv.org/abs/2510.21614). Finds that a coding agent's benchmark score is a poor proxy for how well its descendants will improve, and guides the self-modification tree by a clade-level estimate instead; outperforms prior self-improving coding agents in less wall-clock time.
...
- **SIA: Self Improving AI with Harness & Weight Updates** — 2026. [arXiv:2605.27276](https://arxiv.org/abs/2605.27276). One feedback agent updates both the harness and the weights of a task agent; using both levers beats scaffold iteration alone on legal classification, GPU kernels, and single-cell denoising.
- **Hierarchical Self-Improvement: A Framework for Task-Specific Evolvable Agent Harnesses** — 2026. [arXiv:2608.08466](https://arxiv.org/abs/2608.08466). A frozen model runs a task harness, an evolver that rewrites it, and a meta-evolver that rewrites the evolver. States its own bounds — evolution needs informative reward and cannot exceed the backbone — and reports no improvement on NLE, where the backbone falls short.
...
- **DarwinX: Evolving Agent Harnesses Through Natural Selection** — 2026. [arXiv:2608.07545](https://arxiv.org/abs/2608.07545). Selection over a population of harnesses with the model frozen: a variant is admitted only if it extends coverage without regressing, and an archive keeps alternative lineages for recombination. A harness evolved on Terminal-Bench 2.1 transfers unchanged to SWE-bench Verified.
- **JIT-Agent: Scaling Harness Intelligence via Just-in-Time Harness Evolution** — 2026. [arXiv:2608.25593](https://arxiv.org/abs/2608.25593). A model trained to generate a task-specific harness on the fly for any off-the-shelf LLM, and to repair and evolve it from an archive of earlier harness configurations.
...
- **Continual Harness: Online Adaptation for Self-Improving Foundation Agents** — 2026. [arXiv:2605.09998](https://arxiv.org/abs/2605.09998). Grew out of Gemini Plays Pokemon: a reset-free harness in which an embodied agent alternates between acting and refining its own prompts, sub-agents, skills, and memory, recovering most of the gap to a hand-engineered harness from a bare interface.
...
- **RRSI: Regularized Recursive Self-Improvement of Agent Harnesses** — 2026. [arXiv:2609.24972](https://arxiv.org/abs/2609.24972). Harness self-improvement overfits the tasks it evolves on. Regularizing proposals (an annealed edit budget, a push toward unexplored changes) and selection (a critic and a pruner) keeps gains of up to 4.7 points on five out-of-distribution benchmarks, against 14.1 in-distribution, with 30% fewer policy tokens. From Google Research.
- **SoL-Pi: Recursively Scaling Auto-Research Loops for Efficient Agent Harness** — 2026. [arXiv:2609.20519](https://arxiv.org/abs/2609.20519). Scales auto-research loops across many environments until the harness changes that survive also transfer; the result matches the Pi harness on EdgeBench while cutting token traffic by 44.7–49.0%.
- **Harness-Zero: Harness Distillation via Agent-as-Harness** — 2026. [arXiv:2609.24974](https://arxiv.org/abs/2609.24974). Distils an evolved harness into the weights: a harnessing agent corrects the student before execution, and fine-tuning on the result lets the specialised harness be removed — 23.3% to 44.3% task success, above the 41.7% reached with the harness still attached.
...
- **Evo-Bench: Can Language Models Improve Agent Harness?** — 2026. [arXiv:2608.09096](https://arxiv.org/abs/2608.09096). Measures a model's ability to improve its own harness across search, office, and general tasks, with splits built to separate harness gains from base-model strength.
